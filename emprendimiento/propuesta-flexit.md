# Propuesta de Flexit — tarifario Septiembre 2026

*Analizada el 28/08/2026, sobre el PDF "Tarifario Flexit Septiembre26" que mandó Juan.*
*Dato agregado por Juan: **el IVA sería del 10,5% pagando en efectivo** (el PDF solo dice "+IVA").*

> ## ✅ DECIDIDO el 28/08/2026 — Flexit es la logística de Corteza
>
> Juan confirmó que **es monotributista**, así que el IVA del flete es **costo puro** y el 10,5% en efectivo **sí es un ahorro real** (~$74.837 al mes a 30 envíos por jueves). También confirmó que **el pago sería una vez por mes**, no semanal.
>
> **Las tarifas ya están cargadas en la memoria**: `perfil.md` y `envios-amba.md` (precios por zona, qué cobrarle al cliente y umbrales de envío sin cargo, todos recalculados).
>
> ### Actualizado el 28/08 con lo que Juan confirmó después
>
> - ✅ **Flexit sabe que son panificados a temperatura ambiente.** La pregunta eliminatoria está contestada.
> - ✅ **Mínimo: 120 envíos por MES**, no 30 por despacho. **Es la condición que Smart Post nunca dio.** Pero no es "sin mínimo": mueve el punto de equilibrio de 8 a 49 pedidos por mes. Análisis en `envios-amba.md` sección 5 y en `numeros.md`.
> - ✅ **Pago del total en efectivo, una vez por mes.**
> - 🔥 **Dato nuevo que abre la pregunta más cara: cada pedido va en 2 bolsas camiseta de ~2 kg.** Son **dos bultos por pedido**. Si Flexit cobra por bulto y no por envío, el flete se duplica y **el AMBA no cierra en ninguna zona salvo CABA**. Es lo primero que hay que preguntar.
> - 🔥 **Y la segunda: ¿cómo se cuenta el primer mes?** Arrancando el 17/9 hay solo dos despachos en septiembre; si el mínimo de 120 corre igual, harían falta 60 pedidos por jueves desde el día uno.

---

## 1. El resumen en cinco líneas

- **Es la primera cotización real que entra.** Smart Post nunca puso nada por escrito; Flexit sí.
- **El precio empata con Smart Post**, y empata bien: es **más barato justo en las dos zonas que más importan** (San Isidro–Beccar–Acassuso y Nordelta) y más caro en CABA y en Pilar.
- **Resuelve tres preguntas que estaban abiertas hace un mes**: en qué zona caen Nordelta, Beccar y Acassuso.
- **Deja sin contestar las dos preguntas eliminatorias**: si llevan panificados y si cobran por peso volumétrico. Sin eso, la tarifa es de folleto.
- **Lo del 10,5% en efectivo hay que mirarlo con cuidado.** Ahorra plata de verdad si sos monotributista, pero no es un descuento gratis: ver sección 5.

---

## 2. Las tarifas, y qué pasa con cada IVA

Precios del PDF, **sin IVA**, vigentes en septiembre. Flexit aclara que están "alineadas al precio de envíos Flex de ML".

| Zona | Sin IVA | **Con IVA 10,5% (efectivo)** | Con IVA 21% | Lo que te ahorra el efectivo |
|---|---:|---:|---:|---:|
| **Cercana** | $3.942,10 | **$4.356** | $4.770 | $414 |
| **Media** | $5.522,10 | **$6.102** | $6.682 | $580 |
| **Lejana** | $7.102,10 | **$7.848** | $8.594 | $746 |
| **Muy lejana** | $8.287,10 | **$9.157** | $10.027 | $870 |

**El descuento por pagar en efectivo es del 8,7% sobre el precio final.** No es enorme por envío, pero sumado sí: ver sección 5.

---

## 3. El mapa de zonas — acá está lo más útil del PDF

| Zona | Qué incluye |
|---|---|
| **Cercana** | Todos los barrios de Capital Federal |
| **Media** | Avellaneda, Hurlingham, Ituzaingó, Lanús, Lomas de Zamora, Morón, San Fernando, **San Isidro**, General San Martín, Tres de Febrero, Vicente López, La Matanza norte |
| **Lejana** | Almirante Brown, Berazategui, Esteban Echeverría, Ezeiza, Florencio Varela, José C. Paz, La Matanza sur, Malvinas Argentinas, Merlo, Moreno, Quilmes, San Miguel, **Tigre** |
| **Muy lejana** | Berisso, Campana, Cañuelas, **Del Viso**, **Derqui**, Ensenada, **Escobar**, **Garín**, General Rodríguez, Guernica, **Ingeniero Maschwitz**, La Plata (Centro, Norte y Oeste), Luján, Marcos Paz, **Pilar**, San Vicente, **Villa Rosa**, Zárate |

### Las tres respuestas que estábamos esperando

| Pregunta abierta desde el 10/08 | Respuesta |
|---|---|
| **¿Beccar y Acassuso?** | Son San Isidro → **Zona Media, $6.102**. Es la zona #1 del ranking de publicidad y queda **$163 más barata** que con Smart Post. |
| **¿Nordelta?** | Es Tigre → **Zona Lejana, $7.848**. **$348 más barata** que Smart Post. |
| **¿Pilar?** | **Muy lejana, $9.157.** La banda más cara, igual que con Smart Post, pero **$961 peor**. |

**Además amplían cobertura**: Campana, Zárate, Luján, Guernica y La Plata partida en tres no estaban en el mapa de Smart Post.

---

## 4. Flexit contra Smart Post, zona por zona

| Zona | Smart Post | **Flexit (efectivo)** | Diferencia | |
|---|---:|---:|---:|---|
| CABA | $3.919 | $4.356 | **+$437 (+11,2%)** | ❌ peor |
| San Isidro, Vicente López, Morón… | $6.265 | $6.102 | **−$163 (−2,6%)** | ✅ mejor |
| Tigre/Nordelta, Quilmes, Moreno… | $8.196 | $7.848 | **−$348 (−4,2%)** | ✅ mejor |
| **Pilar** y periféricas | $8.196 | $9.157 | **+$961 (+11,7%)** | ❌ peor |

### Cómo leer esto

**Es un empate, pero un empate que te conviene.** Perdés en CABA (que es tu zona más barata igual) y perdés en Pilar (que no ibas a mandar por logística tercerizada de todos modos). Ganás en las dos zonas donde vas a poner la plata de publicidad.

**Y hay algo que vale más que los $437 de CABA: esto está por escrito.** Smart Post nunca mandó un PDF con el IVA aclarado, ni un mapa de zonas, ni confirmó nada de lo que se le preguntó desde el 19 de agosto.

### Pilar sigue sin solución

| | |
|---|---:|
| Ticket promedio en Pilar | $23.677 |
| Margen efectivo (37,9%) | **$8.974** |
| Flete Flexit a Pilar (Muy lejana) | **−$9.157** |
| **Te queda** | **−$183** |

**El envío se come el 102% de la ganancia del pedido promedio de Pilar: perdés plata en cada uno.** Con Smart Post se comía el 91%; con Flexit directamente da negativo.

> **Conclusión que no cambia: Pilar no se resuelve con ninguna logística del AMBA.** La única salida sigue siendo **cotizar un cadete o moto local** para las ~18 entregas de los jueves. Es la tarea más rentable que queda pendiente y sigue sin hacerse.

---

## 5. El IVA al 10,5% en efectivo — leelo entero antes de decidir

### 5.1 Cuánta plata es

Con una mezcla razonable de zonas (40% CABA, 30% Media, 20% Lejana, 10% Muy lejana), el envío promedio sale **$5.483 sin IVA**:

| Volumen | Neto por mes | IVA 10,5% | IVA 21% | **Ahorro del efectivo** | **Efectivo a pagar** |
|---|---:|---:|---:|---:|---:|
| 20 envíos/jueves (87/mes) | $476.986 | $50.084 | $100.167 | **$50.084** | $527.070 |
| **30 envíos/jueves (130/mes)** | $712.738 | $74.837 | $149.675 | **$74.837** | **$787.575** |
| 40 envíos/jueves (173/mes) | $948.490 | $99.591 | $199.183 | **$99.591** | $1.048.081 |

**A 30 envíos por jueves el ahorro es de $74.837 por mes — casi exactamente tus costos fijos ($82.000).** No es poca cosa.

### 5.2 Pero antes hay que contestar una pregunta

> ### ✅ Confirmado el 28/08: Corteza es **monotributista**
>
> **Eso valida el ahorro.** Queda registrado en `perfil.md`. Para entender por qué importa:
>
> - **Si sos monotributista:** el IVA que te cobran es **costo puro**, no lo recuperás nunca. El 10,5% te ahorra los $74.837 mensuales de verdad. **Conviene.**
> - **Si sos responsable inscripto:** el IVA que pagás es **crédito fiscal** — lo descontás del IVA que cobrás en tus ventas. O sea que **el IVA no te cuesta nada**, y el "descuento" del 10,5% **no vale un peso**. Peor: si te facturan la mitad, perdés crédito fiscal y el gasto no te lo podés computar entero. **Ahí conviene pagar 21% con factura A.**
>
> **Como Corteza es monotributista, el ahorro de $74.837 mensuales es real.** Lo que sí hay que consultar con el contador es el punto 3 de la sección 5.3: hasta cuándo te alcanza el monotributo con el volumen del AMBA.

### 5.3 Y hay que entender qué te están proponiendo en realidad

**La alícuota de IVA la define el servicio, no cómo pagás.** Que te la bajen "si pagás en efectivo" no es una condición comercial normal: significa que te van a facturar distinto de lo que realmente te cobran, o que una parte va sin factura.

**Hay dos maneras de que esto funcione, y hay que saber cuál es:**

- **Te facturan todo, con el 10,5% adentro.** La factura dice exactamente lo que pagás. **No hay nada sin documentar** y el riesgo de la alícuota es de ellos.
- **Te facturan una parte y el resto va sin comprobante.** Ahí sí queda un hueco entre lo que pagás y lo que tenés en papel.

> **La pregunta concreta para Flexit: *"¿la factura va a decir el total que les pago, o solo una parte?"*** Si dice el total, tema cerrado.

**Siendo monotributista esto pesa poco**: en monotributo no deducís gastos, así que tener o no la factura del flete no te cambia un peso de impuestos. Lo que sí queda:

1. **El efectivo hay que conseguirlo.** Si el pago es mensual, son ~$788.000 en un solo retiro a 30 envíos por jueves. **Fijate el tope de extracción de tu banco**, porque esa cifra en un día suele pasarse.
2. **El tope de responsabilidad.** *(Corregido el 28/08: lo que respalda un reclamo por carga perdida no es la factura del servicio, es el remito, el seguimiento y el límite de responsabilidad de la empresa.)* **Preguntales hasta qué monto responden por un bulto perdido o dañado.** Casi todas tienen un tope — por eso te ofrecen el seguro aparte. Si el tope es $10.000 y mandás cajas de $28.000, mejor saberlo ahora.
3. **El monotributo tiene techo.** A 30 pedidos por jueves facturarías ~$3,6 millones por mes, más de $43 millones al año. **Eso puede sacarte del monotributo**, y el día que pases a responsable inscripto este análisis se da vuelta: el IVA pasa a ser crédito fiscal y el descuento del 10,5% deja de valer. **Preguntale al contador las dos cosas en la misma consulta.**

---

## 6. Lo bueno del PDF, más allá del precio

| Qué ofrecen | Por qué importa para Corteza |
|---|---|
| ✅ **Segunda y tercera visita incluidas en el precio** | **Esto es mejor que Smart Post.** Con pan fresco y gente que no está en su casa un jueves a la tarde, el reintento gratis vale mucho. |
| ✅ **El IVA está explícito** ("Precios sin IVA", "+IVA") | Es exactamente el error que casi cometemos con Smart Post, que decía una cosa en el PDF y otra de palabra. |
| ✅ **Same-day** como servicio titular, con las 4 zonas en el mismo tarifario | Sugiere que el mismo día cubre todo el AMBA, no solo CABA. **Hay que confirmarlo igual** (fue lo que hizo caer a Shipnow). |
| ✅ **Cobertura más amplia** | Campana, Zárate, Luján, La Plata en tres bandas. |
| ⚪ **Seguros opcionales**: 0,5% mercadería en depósito, 0,7% en tránsito, contra robo e incendio | **No lo tomes.** Sobre un pedido de $28.000 son $196 por envío, y lo que transportás es pan: si se pierde, se repone. A 130 envíos/mes serían $25.480 anuales… por nada. |
| ⚠️ **"Mantenemos tu reputación en MELI"** | El PDF está escrito para vendedores de MercadoLibre: hablan de reputación en MELI y de tarifas alineadas a Flex de ML. **Hay que confirmar que atienden tiendas propias con Tiendanube**, que es tu caso. |

---

## 7. Lo que el PDF NO contesta — y sin esto no se puede cerrar

Del pliego de 11 preguntas de `logistica-alternativas.md`, el PDF contesta **3**. Faltan las importantes:

| # | Pregunta | Estado |
|---|---|---|
| 1 | **¿Llevan panificados a temperatura ambiente?** (sin cadena de frío ni congelados) | ✅ **Contestada el 28/08: sí, Flexit lo sabe y lo acepta.** |
| **0** | 🔥🔥 **¿Cobran por envío o por bulto?** *(agregada el 28/08 — cada pedido son 2 bolsas camiseta de ~2 kg)* | ❌ **Sin respuesta. Es la más cara de todas.** Si es por bulto, el flete se duplica: $12.204 en zona Media contra $10.612 de margen. |
| 2 | **¿El same-day es en las 4 zonas o solo en CABA?** | ⚠️ Parece que sí, pero **no está dicho**. También eliminatoria. |
| 3 | **¿Peso real o volumétrico? ¿Qué divisor?** | ❌ Sin respuesta. Hoy son 2 bolsas camiseta de ~2 kg (4 kg por pedido). **Si se pasa a caja consolidada, hay que dar las medidas y volver a preguntar.** |
| 4 | ¿Volumen mínimo por despacho o por mes? | ✅ **Contestada: 120 por MES.** Falta saber **cómo se cuenta el primer mes** y **si hay tope máximo**. |
| 5 | ¿Colectan en Pilar? ¿A qué hora y con qué costo? | ❌ **Sin respuesta, y acá hay un riesgo escondido:** Pilar es "Muy lejana" para ellos. Si cobran la colecta, puede salir cara. |
| 6 | ¿Las tarifas incluyen IVA? | ✅ **Contestada.** Sin IVA, +IVA aparte. |
| 7 | ¿Entran a barrios cerrados? | ❌ Sin respuesta. Importa para Nordelta y los countries de Pilar. |
| 8 | ¿Se puede comprometer franja horaria? | ❌ Sin respuesta. Es tu diferencial. |
| 9 | Tarifario y mapa de zonas | ✅ **Contestada, y bien.** |
| 10 | Facturación, plazo de pago, depósito o garantía | ⚠️ A medias: apareció lo del efectivo, falta todo lo demás. |
| 11 | ¿Integran con Tiendanube? | ❌ Sin respuesta en el PDF. |
| **12** | **¿Hasta qué monto responden por un bulto perdido o dañado?** *(agregada el 28/08 — es lo que respalda un reclamo, no la factura)* | ❌ Sin respuesta. |
| **13** | **¿La factura cubre el total que se paga, o solo una parte?** | ❌ Sin respuesta. |

---

## 8. Lo que puede cambiar el lanzamiento entero

**Si Flexit confirma que no tiene volumen mínimo, eso vale más que cualquier diferencia de precio.**

Toda la estrategia del 17/9 está construida sobre el mínimo de 30 envíos de Smart Post:

- la regla de no lanzar con menos de 100 anotados,
- el objetivo de 150 para no pagar envíos fantasma,
- la urgencia de meter $120.000 en Meta para llenar la lista.

**Sin mínimo, esas tres reglas se caen.** Podés salir el 17/9 con 25 pedidos y no pagar un peso de más, y crecer tranquilo. El riesgo más grande del lanzamiento desaparece.

**Confirmado el 28/08: el mínimo es de 120 por mes.** Es mucho mejor que 30 por despacho — un jueves flojo se compensa con uno bueno — **pero no es lo mismo que no tener mínimo.** Mueve el equilibrio de 8 a 49 pedidos mensuales, y en un mes de 60 pedidos el flete te sale $12.117 por envío en vez de $6.058.

**La regla de "no lanzar con menos de 100 anotados" se relaja pero no desaparece:** ya no necesitás 30 el día 1, necesitás ~50 en el primer mes para no perder plata y 120 para estar cómodo. Y falta resolver cómo se cuenta septiembre, que solo tiene dos despachos.

---

## 9. Qué hacer ahora, en orden

1. **Contestale a Flexit hoy** con las 6 preguntas que faltan (texto listo abajo). Es la única cotización real que tenés: no la dejes enfriar.
2. **Consultale al contador** lo del IVA al 10,5% antes de aceptarlo. Preguntá exactamente esto: *"¿me conviene el 10,5% en efectivo o el 21% con factura, según mi condición fiscal?"*
3. **Seguí cotizando igual.** Una cotización sola no es un mercado. GT Pack, Welivery, BNZ y Envíopack siguen sin contestar.
4. **Cotizá el cadete de Pilar.** Flexit confirma que Pilar por logística tercerizada da pérdida. Es la tarea pendiente más rentable que hay.
5. **No cargues todavía los precios nuevos en Tiendanube.** Hasta que no esté resuelto el peso volumétrico, la tarifa puede cambiar.

### Si Flexit cierra, estos son los precios de envío a cargar en Tiendanube

*(costo con IVA 10,5%, recargado por la comisión de Pago Nube del 7% — se divide por 0,93, no se multiplica por 1,07)*

| Zona | Costo real | Exacto | **A cobrar** |
|---|---:|---:|---:|
| CABA | $4.356 | $4.684 | **$4.700** |
| Media (San Isidro, Vicente López…) | $6.102 | $6.561 | **$6.600** |
| Lejana (Nordelta, Tigre, Quilmes…) | $7.848 | $8.439 | **$8.500** |
| Muy lejana (Pilar, Escobar…) | $9.157 | $9.847 | **$9.900** |

> ⚠️ **Ojo con una inconsistencia de la memoria:** `envios-amba.md` calcula el recargo con una comisión de Pago Nube del **6%** (÷0,94), pero `perfil.md` y `numeros.md` dicen **7%**. Acá usé 7%, que es lo que dice el modelo vigente. **Hay que confirmar cuál es el número real** y unificarlo: la diferencia es de unos $50 por envío.

**Y los pisos de envío sin cargo bajan bastante**, porque el margen real (37,9%) es mejor que el 24% con el que se armó la tabla vieja:

| Zona | Piso real (ganás cero) | Umbral sugerido |
|---|---:|---:|
| CABA | $11.493 | **$18.000** |
| Media | $16.100 | **$22.000** |
| Lejana | $20.707 | **$28.000** |
| Muy lejana | $24.162 | bonificar $3.000 desde $30.000 |

---

## 10. Texto para mandarle a Flexit

> Hola, gracias por el tarifario. Me sirve mucho, sobre todo el mapa de zonas.
>
> Les cuento en qué estamos: somos **Corteza** (cortezapan.com.ar), tienda online de panificados artesanales. Lanzamos entrega en todo el AMBA en septiembre, **un solo día por semana (jueves)**, con colecta en **Pilar** cerca del mediodía y entrega el mismo día. Volumen inicial de **30 a 60 envíos por jueves**.
>
> Para poder cerrar necesito que me confirmen por escrito seis cosas:
>
> 1. **¿Transportan panificados a temperatura ambiente?** No llevan cadena de frío ni congelado: es pan, se despacha y se entrega el mismo día.
> 2. **¿El same-day aplica a las cuatro zonas del tarifario**, o solo a Cercana y Media?
> 3. 🔥 **¿La tarifa es por envío o por bulto?** Cada pedido lo despacho en **2 bolsas camiseta de aproximadamente 2 kg cada una** (unos 4 kg en total, misma dirección, mismo destinatario). Necesito saber si eso cuenta como un envío o como dos. **Si es como dos, puedo consolidarlas en una sola caja: díganme si con eso queda como un solo envío y qué medidas máximas admiten.**
> 3 bis. **¿Cobran por peso real o volumétrico?** Si es volumétrico, ¿qué divisor usan?
> 4. Sobre el **mínimo de 120 envíos mensuales** que me confirmaron: **¿cómo se cuenta el primer mes?** Arranco el jueves 17 de septiembre, así que en septiembre tendría solo dos despachos. ¿Es proporcional, o el conteo empieza con el primer despacho? **¿Y hay tope máximo de envíos por mes?**
> 5. **¿Hacen colecta en Pilar? ¿A qué hora y con qué costo?** Pilar les figura como zona Muy lejana y necesito saber si eso afecta la colecta.
> 6. **¿Entran a barrios cerrados y countries o entregan en la guardia?** ¿Se puede comprometer una franja horaria al cliente?
> 7. **¿Hasta qué monto responden por un bulto perdido o dañado?** Mis cajas valen alrededor de $28.000.
>
> Dos consultas más:
>
> - **¿Se integran con Tiendanube?** ¿La integración cotiza sola en el checkout?
> - Sobre la forma de pago: me comentaron la opción de **IVA 10,5% pagando en efectivo**, con pago mensual. **¿La factura va a decir el total que les pago, o solo una parte?** Y para comparar: ¿cuál sería la condición pagando por transferencia con factura completa?
>
> Si pueden, mándenme también el precio por envío para **30, 50 y 80 pedidos en un mismo día**, así veo si baja por volumen.
>
> Gracias.
> Juan Martín Guerrini — Corteza
